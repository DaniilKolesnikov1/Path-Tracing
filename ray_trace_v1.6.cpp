...
std::vector<long double> Trace(const Ray& ray, int n, int m, const Point& pov, const std::vector<Lamp>& lamps, const std::vector<Surface*>& objects, const std::vector<int>& sun, const std::vector<int>& atmos) {
    std::vector<long double> res = {0, 0, 0};//{0.4, 0.4, 0.4};
    ...
    if (min_dist != -1 && min_dist < objects.size()) {
        ...
        if (objects[q]->roughness != 0) {
            int prec = abs(objects[q]->roughness / 0.2);
            ...
            while (!reflected_ok) {
                ...
                //first.is_copied++;// = first.max_copied / 2;
            std::vector<long double> add = Trace(first, n, m, pov, lamps, objects, sun, atmos);
...
std::pair<std::vector<Lamp>, std::vector<Surface*>> CreateScene() {
    ...
    Point centre = Point({-2, -3.2, 4.5});
    Cube* cube = new Cube(centre, {Vector({1, 0, 0}), Vector({0, 1, 0}), Vector({0, 0, 1})}, {0, 1, 1}, 6.4, 0, 1.3, {0.2, 0.2, 0.7});
    Point centre1 = Point({-4, -9.2, -3.5});
    Cube* cube1 = new Cube(centre1, {Vector({1, 0, 0}), Vector({0, 1, 0}), Vector({0, 0, 1})}, {0, 1, 1}, 6.4, 0, 1.3, {0.2, 0.2, 0.7});
    Point centre2 = Point({-4, 2.8, -3.5});
    Cube* cube2 = new Cube(centre2, {Vector({1, 0, 0}), Vector({0, 1, 0}), Vector({0, 0, 1})}, {0, 1, 1}, 6.4, 0, 1.3, {0.2, 0.2, 0.7});
    Sphere* sphere1 = new Sphere(Point({2, -0.4, 0}), 0.3, {0.9, 0.9, 0.9}, 0, 1.4, {0.9, 0.9, 0.9});
    Sphere* sphere2 = new Sphere(Point({3.3, 0.2, -0}), 0.3, {0.3, 1, 1}, 0.9, 0.4);
    Plane* plane = new Plane(Point({10, 0, 0}), Vector({1, 0, 0}), {0.5, 0.4, 0.3}, 0.9, 1.3);
    Plane* plane1 = new Plane(Point({0, 5, 0}), Vector({0, 1, 0}), {0.3, 1, 0.3}, 0.9, 1.3);
    Plane* plane2 = new Plane(Point({0, -5, 0}), Vector({0, 1, 0}), {0.7, 0.8, 1}, 0.9, 1.3);
    Plane* plane3 = new Plane(Point({0, 0, -0.3}), Vector({0, 0, 1}), {0.5, 0.3, 0.3}, 0.9, 1.3);
    Plane* plane4 = new Plane(Point({0, 0, 5}), Vector({0, 0, 1}), {0.5, 0.5, 1}, 0.9, 1.3);
    res.first = {Lamp(cube, 0, 0, 1), Lamp(cube1, 0, 1, 0), Lamp(cube2, 1, 0, 0)};
    res.second = {sphere1, sphere2, plane, plane1, plane2, plane3, plane4};
    return res;
}
